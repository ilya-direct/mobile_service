...
class m160827_070500_create__order__table extends Migration
{
    ...
    public function safeUp()
    {
        $this->createTable('{{%order_person}}', [
            ...
            'address' => $this->string()->comment('Адрес проживания'),
        ]);

        $this->createTable('{{%order_status}}', [
            'id' => $this->primaryKey(),
            'name' => $this->string(100)->unique()->notNull()->comment('Статус заказа'),
        ]);

        $this->createTable('{{%order_provider}}', [
            'id' => $this->primaryKey(),
            'name' => $this->string(100)->unique()->notNull()->comment('Источник заказа'),
        ]);

        $this->createTable('{{%order}}', [
            'id' => $this->primaryKey(),
            'uid' => $this->string(10)->unique()->comment('Номер заказа'),
            ...
            'preferable_date' => $this->date()->comment('Желаемая дата ремонта'),
